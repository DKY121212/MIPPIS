...
def evaluate(model, data_loader):
    ...
    for data in data_loader:
        with torch.no_grad():
            ...
            y_pred = model(node_features, graphs,sequence_names)
...
def test(test_dataframe):
    ...
    for model_name in sorted(os.listdir(Model_Path)):
        print(model_name)
        model = MIPPIS(LAYER, INPUT_DIM, HIDDEN_DIM, NUM_CLASSES, DROPOUT, LAMBDA, ALPHA, VARIANT)
        if torch.cuda.is_available():
            model.cuda()
...
def main():
    ...
    print("Evaluate MIPPIS on Test_60")
    test_one_dataset(Test_60)

    print("Evaluate MIPPIS on Btest_31")
    test_one_dataset(Btest_31)

    print("Evaluate MIPPIS on UBtest_31")
